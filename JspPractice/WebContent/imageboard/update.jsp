...
<%
	...
	if(theme.getPassword()!=null || oldTheme.getPassword().compareTo(theme.getPassword())==0){
			...
			if(imageFileItem.getSize()>0){
				...
				//이미지를 지정한 경로에 저장
				File imageFile = new File("C:\\git\\JspPractice\\WebContent\\image",image);
				//같은 이름의 파일이름 처리
				if(imageFile.exists()){
					for(int i = 0; true; i++){
						imageFile = new File("C:\\git\\JspPractice\\WebContent\\image",image+"_"+i);
						if(!imageFile.exists()){
							image = image +"_"+i;
							break;
						}
				...
				//썸네일 이미지 생성
				File destFile = new File("C:\\git\\JspPractice\\WebContent\\image",image+".small.jpg");
